...
interface PinballChatProps {
  groupId: string;
  messages: PinballChatMessage[];
  isActive: boolean; // ready 또는 playing 상태일 때만 true
}

const PinballChat = ({ groupId, messages, isActive }: PinballChatProps) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await updateDoc(groupRef, {
        'pinballGame.chatMessages': arrayUnion(newMessage),
      });
    } catch (error) {

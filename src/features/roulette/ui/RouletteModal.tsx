...
import { Roulette } from '../components/roulette.ts';
import { doc, updateDoc, arrayUnion } from 'firebase/firestore';
import { db } from '../../../firebase.ts';
import { getAvatarColor, getTextContrastColor, getNextBusinessDay } from '../../../shared/utils';
import RouletteResult from '../components/RouletteResult.tsx';
...
const RouletteModal: React.FC<RouletteModalProps> = ({
    ...
    useEffect(() => {
        if (cart.length > 0) {
            // 대기 중일 때는 실시간으로 캐시 업데이트
            if (status === 'waiting') {
                setCachedCart(cart);
            }
            // 게임 진행 중(ready, playing)인데 캐시가 비어있다면(새로고침 등), 현재 카트로 복구
            else if ((status === 'ready' || status === 'playing') && cachedCart.length === 0) {
                setCachedCart(cart);
            }
        }
    }, [status, cart, cachedCart.length]);
    ...
    const saveRouletteHistory = async (winnerName: string) => {
        // 캐시된 카트가 없으면 현재 카트라도 사용 시도
        const targetCart = cachedCart.length > 0 ? cachedCart : cart;

        if (historySaved || targetCart.length === 0) {
            console.warn('History save aborted: Already saved or empty cart.', { historySaved, cartLen: targetCart.length });
            return;
        }
        ...
        const historyItem: RouletteHistory = {
            ...
            orderItems: cartToHistoryItems(targetCart),
            totalPrice: targetCart.reduce((sum, item) => sum + item.price, 0),
        };

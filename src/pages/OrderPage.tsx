...
const OrderPage = () => {
  ...
  const [selectedSubCategory, setSelectedSubCategory] = useState<string>('전체'); // 소분류 상태 추가
  ...
  // 1. 대분류가 변경되면 소분류를 '전체'로 리셋
  useEffect(() => {
    setSelectedSubCategory('전체');
  }, [selectedCategory]);

  // 2. 현재 대분류에 해당하는 소분류 목록 동적 생성 (useMemo로 최적화)
  const subCategories = useMemo(() => {
    // 현재 대분류에 속한 메뉴들만 필터링
    const menusInUpper = MEGA_MENUS.filter(m => m.categoryUpper === selectedCategory);
    // categoryLower만 뽑아서 중복 제거
  ...
  return (
    <div className="h-full flex flex-col bg-background relative">
      <div className="bg-surface sticky top-0 z-10 shadow-sm">
        ...
        {/* 2차 카테고리 (소분류) - 구분선 추가 */}
        <div className="flex overflow-x-auto space-x-2 no-scrollbar px-6 py-3 border-t border-gray-100 bg-gray-50/50">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-32 custom-scrollbar">
        <div className="grid grid-cols-2 gap-4">
          {/* 필터링 로직: 대분류 일치 AND (소분류가 '전체'이거나 소분류 일치) */}
          {MEGA_MENUS

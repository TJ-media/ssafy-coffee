/**
 * 메가커피 메뉴 데이터 및 Firestore 업로드 유틸리티
 * 
 * 사용법: 브라우저 콘솔에서 uploadMegaData() 호출
 * 또는 임시 버튼을 통해 실행
 * 
 * 이 스크립트는 일회성 마이그레이션용입니다.
 */
import { doc, setDoc } from 'firebase/firestore';
import { db } from '../../firebase';

export const uploadMegaData = async () => {
    const megaData = {
        cafeId: 'mega',
        cafeImg: '🟡',
        cafeName: '메가커피',
        categories: ['메뉴 추가', '커피', '디카페인', '음료', '티', '푸드', '상품'],
        items: [
            // --- 1. 커피 > 에스프레소 ---
    ...
    try {
        // Firestore의 'menus' 콜렉션 내 'mega' 문서에 데이터 저장 (덮어쓰기)
        await setDoc(doc(db, 'menus', 'mega'), megaData);
        alert('메가커피 메뉴가 성공적으로 업로드되었습니다!');
    } catch (error) {
        console.error('메가커피 데이터 업로드 실패:', error);
